...
def getType(browser):
    ...
    im0 = Image.open(StringIO(browser.get_screenshot_as_png()))
...
def get_cookies(login_name,password):
    ...
    login = browser.find_element_by_id('loginAction')
    name.send_keys(login_name)  # 测试账号
...
if __name__ == '__main__': 
    option = webdriver.ChromeOptions()
    #prefs={"profile.password_manager_enabled": 0}
    #option.add_experimental_option("prefs", prefs)
    #option.add_argument('--user-data-dir=/home/wj/.config/chromium/Default/') 
    ...
    psw = browser.find_element_by_id('loginPassword')
    login = browser.find_element_by_id('loginAction')
    name.send_keys('573510848@qq.com')  # 测试账号
    psw.send_keys('jsy1314')
    login.click()
